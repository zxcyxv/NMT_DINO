...
def create_kaggle_notebook():
    with open("phase1_dino_ema_train_single_gpu.py", "r", encoding="utf-8") as f:
    ...
    add_md(
        ...
        "on a single GPU, built for direct execution on Kaggle."
    )

    # ── Cell 2: Install dependencies ───────────────────────────
    add_md("## 1. Install Dependencies")
    add_code("!pip install -q transformers sacrebleu kagglehub")

    # ── Cell 3: Write training script ──────────────────────────
    add_md(
        "## 2. Write Training Script\n"
        "Write the training logic to `train_script.py` and run it via the OS shell "
...
"""
    ...
    add_md(
        ...
        "Single GPU training with plain `python`. "
        "Adjust `--epochs`, `--batch_size`, `--grad_accum` as needed."
    )
    add_code(
...
"""
    ...
    out_name = "Kaggle_DINO_EMA_SingleGPU.ipynb"
